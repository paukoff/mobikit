	...
	var Slider = function(element, options) {
		...
		switch(this.orientation) {
			...
			default:
				...
				this.tooltip.add([this.tooltip_min, this.tooltip_max]).addClass('top');
				break;
		}
		...
		layout: function(){
			...
			if (this.orientation === 'vertical') {
                ...
                if (offset_min.right > offset_max.left) {
                    ...
                    this.tooltip_max.addClass('top');
                }
			}

			if (this.range) {
				if (this.element.attr('data-format') == 'т.р.') {
					this.tooltipInner.text(
						'от ' + this.formater(this.value[0]) + ' - ' + 'до ' + this.formater(this.value[1])
					);
				} else {
					this.tooltipInner.text(
				...
				if (this.element.attr('data-format') == 'т.р.') {
					this.tooltipInner_min.text(
						'от '+this.formater(this.value[0])
					);
	                this.tooltipInner_max.text(
						'до '+this.formater(this.value[1])
					);
				} else {
					this.tooltipInner_min.text(
				...
				this.tooltip_min[0].style.cssText = 'left: ' + positionPercentages[0]+'%;';
				this.tooltip_max[0].style.cssText = 'left: ' + positionPercentages[1]+'%;';
